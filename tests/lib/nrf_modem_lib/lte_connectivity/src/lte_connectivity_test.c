...
void test_initial_settings_match_kconfig(void)
{
	TEST_ASSERT_EQUAL(
...
/* Verify that conn_mgr_if_set_opt successfully changes the IF_DOWN option */
void test_options_set_should_change_option_value(void)
{
	struct net_if *net_if = net_if_get_default();
	...
	/* Attempt to change the IF_DOWN option */
	network_interface_down_option_set(LTE_CONNECTIVITY_IF_DOWN_MODEM_SHUTDOWN, 0);
	...
	/* Attempt to change the IF_DOWN option back to its original value */
	network_interface_down_option_set(LTE_CONNECTIVITY_IF_DOWN_LTE_DISCONNECT, 0);

	/* Verify that the IF_DOWN option was successfully changed again */
	len = sizeof(option);
	ret = conn_mgr_if_get_opt(net_if, LTE_CONNECTIVITY_IF_DOWN, (void *)&option, &len);
	TEST_ASSERT_EQUAL(LTE_CONNECTIVITY_IF_DOWN_LTE_DISCONNECT, option);
	TEST_ASSERT_EQUAL(0, ret);
...
void test_pdn_triggers_ipv4_address_changes(void)
{
	struct net_if *net_if = net_if_get_default();
...
void test_pdn_does_not_add_nonexistant_ipv4(void)
{
	struct net_if *net_if = net_if_get_default();
...
void test_pdn_triggers_ipv6_address_changes(void)
{
	struct net_if *net_if = net_if_get_default();
...
void test_pdn_does_not_add_nonexistant_ipv6(void)
{
	struct net_if *net_if = net_if_get_default();
...
/* Verify that the iface is inactive before PDN and CEREG are ready, becomes active
 * afterwards, and becomes inactive again if either is lost.
 *
 * Verify also that LTE_LC_NW_REG_SEARCHING is ignored
 *
 * k_sleep calls are not strictly necessary since iface state changes are instantaneous,
 * but we perform them so that the NET_MGMT event queue is not overflowed during the test.
 */
void test_iface_pdn_and_cereg_tracking(void)
{
	struct net_if *net_if = net_if_get_default();
