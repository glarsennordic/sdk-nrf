					   ...
					   struct nrf_cloud_data *const output)
{
	 ...
	 * "state":{"config":{"activeMode":false}},
	 * "metadata":{"config":{"activeMode":{"timestamp":1695404646}}}}
	 */
	...
	/* If we accept the shadow values, update reported; otherwise,
	 * update desired to indicate the values are not valid.
	 */
	response_obj = cJSON_AddObjectToObjectCS(state_obj,
						 accept ? NRF_CLOUD_JSON_KEY_REP :
							  NRF_CLOUD_JSON_KEY_DES);

	/* When accepting the change, add the delta to reported:
	 *   accept: {"state":{"reported":{"config":{"activeMode":false}}}}
	 * When rejecting, either set the desired to null or the last valid value,
	 * such as if sent a negative value where only positive values are valid:
	 *   reject: {"state":{"desired":{"config":{"locationTimeout":400}}}}
	 */
	 output_obj = cJSONUtils_MergePatch(response_obj, delta_obj);
					     ...
					     struct nrf_cloud_data *const output)
{
	...
	cJSON *reported_obj = NULL;
	cJSON *desired_obj = NULL;
	cJSON *rcontrol_obj = NULL;
	cJSON *dcontrol_obj = NULL;

	if (!IS_ENABLED(CONFIG_NRF_CLOUD_COAP)) {
		state_obj = cJSON_AddObjectToObjectCS(root_obj, NRF_CLOUD_JSON_KEY_STATE);
		if (!accept) {
			desired_obj = cJSON_AddObjectToObjectCS(state_obj, NRF_CLOUD_JSON_KEY_DES);
			dcontrol_obj = cJSON_AddObjectToObjectCS(desired_obj,
								 NRF_CLOUD_JSON_KEY_CTRL);
			alert_ret = json_add_null_cs(dcontrol_obj, NRF_CLOUD_JSON_KEY_ALERT);
			log_ret = json_add_null_cs(dcontrol_obj, NRF_CLOUD_JSON_KEY_LOG);
		}
	...
	rcontrol_obj = cJSON_AddObjectToObjectCS(reported_obj, NRF_CLOUD_JSON_KEY_CTRL);
	alert_ret = json_add_bool_cs(rcontrol_obj, NRF_CLOUD_JSON_KEY_ALERT,
				     data->alerts_enabled);
	log_ret = json_add_num_cs(rcontrol_obj, NRF_CLOUD_JSON_KEY_LOG, data->log_level);

	/* If these objects could not be created, we ran out of memory */
	if (!(root_obj && rcontrol_obj && !alert_ret && !log_ret)) {
		err = -ENOMEM;
		goto end;
	}
