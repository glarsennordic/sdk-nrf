...
#if defined(CONFIG_NRF_CLOUD_COAP_SEND_SSIDS)
...
#else
		dst->_ap_ssid_present = false;
#endif /* CONFIG_NRF_CLOUD_COAP_SEND_SSIDS */
		num_aps--;
		if (!num_aps) {
			break;
		}
...
#if defined(AGPS_DEBUG)
		LOG_DBG("num elements: %d", t->_agps_req_types_int_count);
		for (int i = 0; i < t->_agps_req_types_int_count; i++) {
			LOG_DBG("  %d: %d", i, t->_agps_req_types_int[i]);
		}
#endif
	} else {
