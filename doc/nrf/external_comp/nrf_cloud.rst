...
* Each CoAP transaction contains a CoAP header and any API specific payload.
...
* The public key derived from the private key is registered with an nRF Cloud account.
  The device will be visible in the nRF Cloud account portal, including location service data and sensor data, if the public key is also associated with the same nRF Cloud account.
...
* Just in time provisioning
  ...
  This is supported by MQTT or REST connections, but not CoAP connections.
